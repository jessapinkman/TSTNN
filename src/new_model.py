...
class Net(nn.Module):
    def __init__(self, L=512, width=64):
        ...
        self.width = width
        #小波变换
        #self.inp_wavelet1 = wavelet.Haar2()
        #self.inp_wavelet2 = wavelets.DTCWTForward(J=3, biort='near_sym_b', qshift='qshift_b')
        #self.inp_wavelet3 = wavelets.DTCWTInverse(biort='near_sym_b', qshift='qshift_b')
        #kernel_size()设置卷积核的长度和宽度
    ...
    def forward(self, x):
        ...
        #out_low, out_high = self.inp_wavelet2(x)
        #out = self.inp_wavelet3((out_low, out_high))
        out = self.inp_conv(x)        
        out = self.inp_prelu(self.inp_norm(out))
        
        out = out + self.enc_dense1(out)   # [b, 64, num_frames, frame_size]

        x1 = self.enc_conv1(self.pad1(out)) # [b, 64, num_frames, 256]
        
        x1 = self.enc_prelu1(self.enc_norm1(x1))

        out = self.dual_transformer(x1)  # [b, 64, num_frames, 256]
        #two path:Tanh、Sigmoid
        out = self.output1(out) * self.output2(out)  # mask [b, 64, num_frames, 256]

        out = self.maskrelu(self.maskconv(out))  # mask
        #encoder的输出和masking modelu的输出相乘
        out = x1 * out
        #Dense Block and Sub-Pixed conv
